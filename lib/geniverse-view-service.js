...
$('#addDragon').click(function(e) {
       if (GenView.mySelectedDragon != null)
      Chat.addMessageObjectToChat(GenView.mySelectedDragon);
});

$('#addDragonToBreeding').click(function(e) {
  if (GenView.mySelectedDragon != null){
    var parent;
    if (GenView.mySelectedDragon.dragon.sex == 0) {
      GenView.father = GenView.mySelectedDragon;
      parent = "father";
    } else {
      GenView.mother = GenView.mySelectedDragon;
      parent = "mother";
    }
...
$('#addChildToPool').click(function(e) {
  if (GenView.child != null){
    addDragonToMyPool(GenView.child)
    GenView.child = null;
  }
});
...
$('#addDragonToMyPool').click(function(){
  if (GenView.publicSelectedDragon != null){
    addDragonToMyPool(GenView.publicSelectedDragon);
  }
});

$('#breed').click(function(e) {
  if (GenView.mother != null && GenView.father != null){
      if (GenGWT.isAlive(GenView.mother.dragon) && GenGWT.isAlive(GenView.father.dragon)){
        var onSuccess = function(dragon) {
          GenView.child = new DragonBundle();
          GenView.child.dragon = dragon;
          GenView.child.mother = GenView.mother;
          GenView.child.father = GenView.father;
          GenView.child.image = dragon.imageURL;
          var dragonImage = jQuery(document.createElement('img'))
          dragonImage.hide();
          dragonImage.attr('id', 'childImage');
          addImage('childImage', GenView.child.image, 74, 74);
          $('#child').hide();
...
function addDragonToMyPool(dragonBundle) {
  ...
  image.click(function (e){
    ...
    GenView.mySelectedDragon = dragonBundle;
  });
...
function addDragonToPublicPool(dragonBundle) {
  ...
  image.click(function (e){
    var img = image.attr('src');
    var dragonBundle = GenView.allDragons[image.attr('id')];
    GenView.publicSelectedDragon = dragonBundle;
  });
