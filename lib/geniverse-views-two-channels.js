...
function addDragonToPool(dragon, imageurl, divid, classname) {
 ...
 //   dragon.metaInfo.put("xx","yy")
 //   alert(JSON.stringify(dragon.metaInfo))
 //   if(dragon.metaInfo == null || dragon.metaInfo.get('id') == null){
        var dragonId = dragon.name_0 + dragon.alleles
        dragonId = dragonId.replace(/[ :,]/g,"")
        dragon.metaInfo.put("id",dragonId)
 //   } else {
 //       var dragonId = dragon.metaInfo.get('id');
 //   }
    
    // rm existing image with same id
    if (allDragons[dragonId] != null){
	...
	tooltip = { 
	    ...
	    bodyHandler: function() { 
	        ...
	        tip.append(img);
	        tip.append("<br>");
	        tip.append(dragon.metaInfo.get("chat"))
	        return tip; 
	    } 
	}
